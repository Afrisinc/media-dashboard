...
const postSchema = z.object({
  ...
  message: z.string().min(1, "Message is required").max(63206),
...
const SocialMediaPostForm = () => {
  ...
  const buildPayload = (data: PostFormData) => {
    ...
    // Use carousel images if available, otherwise use single image
    const mediaUrls =
    ...
    return {
      ...
      media:
        mediaUrls.length > 0
          ? {
              type: (mediaUrls.length > 1 ? "carousel" : "image") as
                | "carousel"
                | "image"
                | "video",
              urls: mediaUrls,
                  ...
                  {currentPlatform ? (
                    pagesLoading ? (
                      ...
                      <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-sm text-amber-700 dark:text-amber-200">
                        No connected accounts for this platform. Please connect
                        an account first.
                      </div>
                    )
                  ) : (
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription className="text-xs">
                    {field.value.length}/63206 characters
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
